...
vector<ErlNifBinary> doEncode(unsigned char* data, size_t dataSize, int k, int m, int w, CodingType coding) {
    ...
    long long blockSize = roundTo((roundTo(dataSize, k*w) / (k*w)), 4);
    ...
    for(int i = 0; i < k - 1; ++i) {
        ...
        memcpy(dataBlock.data, data + blockSize * (k-1), blockSize);
    ...
    jerasure_schedule_encode(k, m, w, smart, dataBlocks, codeBlocks, w*blockSize, blockSize);
...
encode_test(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    return enif_make_list_from_array(env, retArr, k + m);
}
